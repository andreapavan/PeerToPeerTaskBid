...
server(User_List) ->
	...
	receive
		...
		{From, message_to, To, Message} ->
			...
			io:format("list is now: ~p~n", [User_List]), server(User_List)
	end.
...
message(ToName, Message) ->
	case whereis(mess_client) of % Test if the client is running
		...
		_ -> mess_client ! {message_to, ToName, Message}, ok
	end.
...
receive
	logoff ->
		{messenger, Server_Node} ! {self(), logoff}, exit(normal);
	{message_to, ToName, Message} -> 
		{messenger, Server_Node} ! {self(), message_to, ToName, Message}, await_result();
